...
interface Props {
  ...
  phase: EventPhase;
  now: Date;
  /** Walkthrough anchors, space-separated; each points at the first card carrying it. */
  tourAnchor?: string;
}
    ...
    >
      ...
      <div className="event__body">
        ...
        <p className="event__start">
          {event.endsAt
            ? formatPeriod(event.startsAt, event.endsAt, now)
            : formatStart(event.startsAt, now)}
        </p>
      </div>
...
function Countdown({ event, phase, now }: { event: GameEvent; phase: EventPhase; now: Date }) {
      ...
      <div className="event__countdown">
        ...
        {event.endsAt && (
          <p className="event__time">
            <span className="event__caption">{COUNTDOWN_LABELS.untilEnd}</span>
            <Clock until={event.endsAt} now={now} />
          </p>
        )}
      </div>
    );
  ...
  return (
    <div className="event__countdown">
      ...
      <p className="event__time">
        <span className="event__caption">{COUNTDOWN_LABELS.untilStart}</span>
